...
Use preprocessing code from exercise #3 but, once you computed proportion per `Participant`×`Prime`×`Probe`, you need to group data over `Prime`×`Probe` to computer average performance across observers as well as 89% confidence interval (think about what are the lower and upper limits for 89% CI). You can compute quantiles using the function of the same name: [quantile()](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/quantile.html). Then, use [geom_point()](https://ggplot2.tidyverse.org/reference/geom_point.html) to plot the mean response and [geom_errorbar()](https://ggplot2.tidyverse.org/reference/geom_linerange.html) to plot 89% CI (you will need to map the two variable you computed to `ymin` and `ymax` properties). The plot should like like this (hint, drop color mapping and map `Prime` to `group` property).
