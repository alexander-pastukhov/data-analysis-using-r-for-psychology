# Repeating a computation and controlling its flow
...
Alternatively, you can repeat each element specified number of times before repeating the next one, is done via `each` parameter. The difference between options lies only in the _order_ of elements in the new vector. As you can see both vectors have the same length and each individual value is repeat four times. 
```{r}
rep(c(1, 2, 3), each=4)
```
...
However, you can also use subsetting of a new vector to achieve same ends.
```{r}
...
To create a table with all combinations of values, you can use either base R [expand.grid()](https://stat.ethz.ch/R-manual/R-devel/library/base/html/expand.grid.html) or tidyr's implementation [expand_grid()](https://tidyr.tidyverse.org/reference/expand_grid.html). The latter is a bit more robust and can expand even tables and matrices (but see the documentation for the differences in implementation and output).
...
[expand_grid()](https://tidyr.tidyverse.org/reference/expand_grid.html) works the same they but for the order in the values very within columns.
```r
