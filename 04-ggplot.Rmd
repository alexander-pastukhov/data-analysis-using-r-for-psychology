...
You always need the first one. But you do not need to specify the other two, even though a plot without geometry in it looks very empty. Let us start with an example using [mpg](https://ggplot2.tidyverse.org/reference/mpg.html) data. Here we will plot engine displacement on x-axis, the measured miles-per-gallon in a city cycle on y-axis, number of cylinders as color, and type of drive train (front-wheel, read-whell, or four-wheel) as shape.

```{r fig.align = 'center'}
# simple_tidy_data <- tibble::tribble(
#   ~
# )
...
ggplot(data=msleep, aes(x=brainwt, y=sleep_total, color=vore)) + 
  geom_point()
```
