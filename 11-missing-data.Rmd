...
To make implicit missing data explicit, _tidyr_ provides a function [complete()](https://tidyr.tidyverse.org/reference/complete.html) that figures out all combinations of values for columns that you specified, finds missing combinations, and adds them using `NA` (or some other specified value) for other columns. Imagine a toy incomplete table (no data for Participant `2` and Face `M-2`).
```{r echo=FALSE}
widish_df <- 
...
knitr::kable(drop_na(widish_df_with_NA),
             caption = "Complete cases via drop_na()")
```
...
widish_df_with_NA %>%
  drop_na(Attractiveness) %>%
  knitr::kable(caption = "Complete Attractiveness via drop_na()")
```
...
It works _per element_, so each value is replaced by its counterpart in the appropriate vector
```{r}
v <- c(0, 0, 1, 0, 1)
ifelse(v == 1,
       c(1, 2, 3, 4, 5), 
...
adapt_df %>%
  ...
  head() %>%
  knitr::kable(caption = "adaptation_with_na.csv with imputed values")
```
