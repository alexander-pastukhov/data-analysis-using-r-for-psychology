...
```{r}
x_total <- 70
x <- rbinom(1, x_total, 0.7)
y_total <- 50
y <- rbinom(1, y_total, 0.5)
xy <- rbind(c(x, x_total - x), c(y, y_total - y))

odds_ratio <- function(x, x_total, y, y_total) {
  (x * (y_total - y)) / (y * (x_total - x))
}



fisher.test(xy)
odds_ratio(x, x_total, y, y_total)


shuffle_and_odds_ratio <- function(x, x_total, y, y_total){
  outcome <- sample(c(rep(1, x+y), rep(0, (x_total - x) + (y_total - y))))
  new_x <- sum(outcome[1:x_total])
  new_y <- sum(outcome[(x_total+1):(x_total+y_total)])
  odds_ratio(new_x, x_total, new_y, y_total)
}

odds <- replicate(10000, shuffle_and_odds_ratio(x, x_total, y, y_total))
cat(glue("Probability of higher odds ratio by chance: {mean(odds > odds_ratio(x, x_total, y, y_total))}"))
```
...
```{r echo=FALSE}
raw_results <-
  fs::dir_ls(path="data/gamification/", glob="*.csv") %>%
