# Pivoting: Tables wide and long {#seminar08}
...
Today, you will learn how tidy and untidy your table but first, let us practice more, utilizing knowledge and skill from previous seminars!  Grab the [exercise notebook](notebooks/Seminar 08.Rmd) and let's get started.
...
There is not much more to know about [glue](https://glue.tidyverse.org/), so its simplicity makes it a great tool to format strings using values. However, because it is so simple to use, it is not the most flexible way for formatting strings. Later, you will learn about [C-style string formatting](https://stat.ethz.ch/R-manual/R-patched/library/base/html/sprintf.html) (same as Python's old string formatting).
