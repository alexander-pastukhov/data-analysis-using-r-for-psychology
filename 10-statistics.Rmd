...
For pairwise comparisons for normally distributed data, you can use Student's t-Test [t.test()](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/t.test.html). 

```{r}
df <- 
  tibble(x = rnorm(100, mean = 1, sd=1)) %>%
  mutate(y = rnorm(100, mean = 1.2, sd = 1))
...
t.test(x ~ group, data=df_group)
```


Alternatively, you have a non-parameteric Wilcoxon Rank Sum and Signed Rank Tests [wilcox.test()](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/wilcox.test.html).
...
```{r}
library(coin)
oneway_test(x~group, data=df_group)
```
