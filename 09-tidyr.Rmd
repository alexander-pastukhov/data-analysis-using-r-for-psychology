...
```{r}
clear_percepts <- 
  read_csv2("data/clear-percepts.csv",
            col_types = cols(
                             Session = col_character(),
                              Participant = col_character(),
                              Block = col_double(),
                              ...
                              Duration = col_double(),
                              iPercept = col_double(),
                              SpeedUp_first = col_logical())) %>%
  group_by(Participant, Condition, Display) %>%
  summarize(Dmedian = median(Duration), .groups="drop") %>%
  ungroup() %>%
  pivot_wider(id_cols=c("Participant", "Condition", "Display"), names_from = "Display", values_from="Dmedian")


ggplot(clear_percepts, aes(x=`KDE 45`, y=`NC`)) + 
  geom_point() +
