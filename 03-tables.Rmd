...
Trying to extract multiple items, use of negative or zero indexes fill all fail for _simplifying_ subsetting.
```{r, eval=FALSE}
# "attempt to select more than one element in vectorIndex" error
x[[1:2]]

# technically, we are excluding two out of three indexes, so end up with just one value
# still, "attempt to select more than one element in vectorIndex"
x[[-(1:2)]]

# A different error message: "attempt to select less than one element in get1index <real>"
x[[0]]
```
...
```{r, eval=FALSE}
# this will generate a "subscript out of bounds" error
x[["d"]]
```
...
Preserving subsetting via `[]` returns a list that is a part of original list, again the _name_ associated with an index so it is retained in the returned list.
```{r}
l[2]
```
...
Using multiple, negative or zero indexes will fail just like it does for vectors but _some_ error messages will be different.
```{r, eval=FALSE}
# "subscript out of bounds" error
l[[1:2]]

# technically, we are excluding two out of three indexes, so end up with just one value
# still, "attempt to select more than one element in integerOneIndex"
l[[-(1:2)]]
...
**General rule:** use `[[]]` if you are interested in list or table content, e.g., you need a column from the table. Use `[]` if you interested in using a smaller list or table.
...
My advice would be to stick to `[[]]` notation despite the ubiquitous use of `$` out where. It is slightly more strict and more universal: You can hard code a column/item name or put it into a variable for indexing. Only hard-coding works for `$`.
...
## Subsetting table {#table-subsetting}
