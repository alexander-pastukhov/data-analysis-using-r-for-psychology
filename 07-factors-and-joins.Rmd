...
You can specify the order of levels either during the `factor()` conversion call or later using [forcats](https://forcats.tidyverse.org/) library (more on that later). For example, if we want to have levels in the reverse order we specify it via `levels` parameter. Note the opposite order of levels.
```{r}
letters <- c("C", "A", "D", "B", "A", "B")
