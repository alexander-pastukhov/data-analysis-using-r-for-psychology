...
When working with strings, I strongly suggest consulting a manual and vignettes of the [stringr](https://stringr.tidyverse.org/index.html) package. It has many functions that cover most needs. 
...
bands_df <- read_csv("data/bands-adaptation.csv",
  ...
  summarise(Pmedian = round(median(Psame), 2),
            ...
            .groups = "drop")
knitr::kable(bands_df, caption="bands_df")
```
...
The table above gives us information about _median_ probability of seeing the same rotation and about its absolute deviation from the median. However, it would be more convenient for a reader we would combine these two pieces of information into a single entry in form for of "<median> ± <MAD>". Plus, it would be easier to see the pattern in a square table with one `Prime` per row and one `Probe` per column. The table I have in mind look like this:
```{r echo=FALSE}
bands_df %>%
...
## paste {#paste}
Base R has functions [paste()](https://stat.ethz.ch/R-manual/R-devel/library/base/html/paste.html) and [paste0()](https://stat.ethz.ch/R-manual/R-devel/library/base/html/paste.html) that are conceptually very similar to [glue()](https://glue.tidyverse.org/reference/glue.html). The concatenate a vector a strings into a single string. If you recall, vector values can [only be of one (most flexible) type](#vectors-are-homogeneous). Therefore, if you have a vector that intersperses string with other values, they will be _first_ converted to strings anyhow. The difference between `paste()` and `paste0()` is that the former puts a separator string in-between each value (defaults to `' '` but you can define your own via `sep` argument), whereas `paste0()` uses no separator. We can replicate our [glue()](https://glue.tidyverse.org/reference/glue.html) example.
```{r}
answer <- 42
bad_answer <- 41
...
substr()](https://stat.ethz.ch/R-manual/R-patched/library/base/html/substr.html) or string processing library [stringr](https://stringr.tidyverse.org/). Alternatively, you can split a string column into several columns via [separate()](https://tidyr.tidyverse.org/reference/separate.html) or [extract()](https://tidyr.tidyverse.org/reference/extract.html) functions.

## Separate
Function [separate()](https://tidyr.tidyverse.org/reference/separate.html) is part of tidyverse and its use is very straightforward: you pass 1) the name of the column that you want to split, 2) names of the columns it needs to be split into, 3) a separator symbol or indexes of splitting positions. Examples using the face table should make it clear. Reminder, this is the original wide table and we want to separate `Face` into `FaceGender` and `FaceIndex`.
...
Let's practice. Use [separate()](https://tidyr.tidyverse.org/reference/separate.html) to preprocess [persistence](data/persistence.zip) data and create two new columns for hour and minutes from `Session` column. Do it in a single pipeline, starting with reading all files (use tidyverse [read_csv()](https://readr.tidyverse.org/reference/read_delim.html) and specify column types!) and renaming `Shape` (`Prime`) and `Shape2` (`Probe`) columns. You results should look like this, think about columns that you drop or keep (this is only first four rows, think of how you can limit your output the same way via [head()](https://stat.ethz.ch/R-manual/R-devel/library/utils/html/head.html) function):
```{r echo=FALSE}
results <-
fs::dir_ls(path = "persistence/") %>%
...
```{r}
widish_df %>%
  separate(Face, into=c("FaceGender", "Dash", "FaceIndex"), sep=c(1, 2), remove=FALSE) %>%
  knitr::kable()
```
...
Base R provides a function extract a substring (or many substrings) via [
substr()](https://stat.ethz.ch/R-manual/R-patched/library/base/html/substr.html) function (you can also its alias `substring()`). It takes a string (or a vector of strings) and vectors with `start` and `stop` indexes of each substring. 
...
Tidyverse has its own [stringr](https://stringr.tidyverse.org/) library for working with strings. Its uses a consistent naming scheme `str_<action>` for its function and covers virtually all tasks that are related to working with strings. [stringr](https://stringr.tidyverse.org/) equivalent of  [
substr()](https://stat.ethz.ch/R-manual/R-patched/library/base/html/substr.html) is [str_sub()](https://stringr.tidyverse.org/reference/str_sub.html) that behaves similarly.
...
When it comes to regular expressions, what I have shown you so far is not even a tip of an iceberg, it is a tip of a tip of an iceberg at best. They are very flexible, allowing you to code very complicated patterns but they are also hard to read and, therefore, hard to debug^[Hence, the quote at the beginning of the section.]. For example, this is a regular expression to check validity on an email address
```
(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*|"(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])*")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[a-z0-9-]*[a-z0-9]:(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21-\x5a\x53-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])+)\])
```

Still, if you need to work with text they are indispensable, so your should remember about them. When facing an actual task grab a [cheatsheet](https://raw.githubusercontent.com/rstudio/cheatsheets/main/strings.pdf) and use an [online expression tester](https://spannbaueradam.shinyapps.io/r_regex_tester/) to debug the pattern. 
...
Now let us practice extracting the relevant information via pattern matching. Use [str_extract()](https://stringr.tidyverse.org/reference/str_extract.html) to extract the participants unique code, the  letters of `Participant` column. Again, here you can simply use a [substr()](https://stat.ethz.ch/R-manual/R-patched/library/base/html/substr.html) but I want you write a pattern that matches one or more upper case letters at the beginning of the string.

::: {.infobox .practice}
Do exercise 10.
:::

Finally, let us practice in replacing part of a string via regular expressions. Use [str_replace()](https://stringr.tidyverse.org/reference/str_replace.html) to anonymize the birth year of our participants. You need to replace the four digits that represent their birth year with a single `"-"`.  The table should look as follows:
```{r echo=FALSE}
results %>%
