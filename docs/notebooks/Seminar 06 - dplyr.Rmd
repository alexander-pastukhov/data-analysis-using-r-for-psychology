...
## Exercise 09: order
Repeat exercises 8 but using `order` instead of `arrange`.
...
Group by `vore` variable and compute 1) average total sleep, 2) 4.5% quantile (see [quantile](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/quantile.html) function or type `?quantile` in console).
```{r exercise 11-1}
```
