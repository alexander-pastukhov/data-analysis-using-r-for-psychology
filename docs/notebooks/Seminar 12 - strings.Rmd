...
## Exercise 09
Using table from previous exercise,  use a regular expression to [filter()](https://dplyr.tidyverse.org/reference/filter.html) out Primes and Probes that _end_ with a _single_ digit. The regular expression pattern must specify that only one digit is allowed and that it must be the last symbol.
...
## Exercise 11
Using same table, use [str_replace()](https://stringr.tidyverse.org/reference/str_replace.html) to anonymize the birth year of our participants. Replace the _four_ digits that represent their birth year with a _single_ `"-"`.
