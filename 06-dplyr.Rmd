...
## `filter()` rows by values {#filter}
[filter()](https://dplyr.tidyverse.org/reference/filter.html) function is complementary to `select()`: the latter slices your table vertically, `filter()` slices it horizontally. The idea is fairly straightforward: You specify condition(s) and you get only the rows that satisfy them. To use filtering properly, you need to understand conditions with logical operators and logical indexing.

### Conditions
In R you create a set of statements about value and they will be evaluated to see whether 
